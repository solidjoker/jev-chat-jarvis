...
**修复**
- 「分析 / 沙盘 / 回复」三个按钮改为常驻：点「分析」后按钮不再消失，进度条和结果显示在按钮下方，出错时按钮同样保留，随时可换另一个动作。
- 进度条改为纯 View 手搓实现：之前依赖系统主题的进度条在暗色模式的 MIUI 上渲染成一条发丝线，等于没有；同时修复加载中面板被聊天消息事件每秒冲掉的问题——这也是「点了分析没反应」的主因。
- 微信「还是不行」现在有明确出路：在微信里点「分析」会直接显示状态卡——未开「通知使用权」时给一键开启按钮；已开启则说明「正在微信里打开的会话不会有通知，退到桌面等对方来一条新消息就会自动分析」。微信仍然绝不截屏。
- 上一轮还在运行时点按钮，从静默改为提示「上一轮还在进行，请稍候」。
- 设置页标题下直接显示当前版本号。

**下载**：[jev-assistant-v1.61-release.apk](https://github.com/jev-chat/jev-chat-jarvis/raw/v1.61/apk/jev-assistant-v1.61-release.apk)
