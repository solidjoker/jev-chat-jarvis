...
- 识别提速与进度可见：飞书等气泡 OCR 从「每个气泡跑一次识别」改为整屏单趟识别后按位置归入气泡（一屏几十条气泡从几十秒降到一次识别的时间）；识别期间悬浮球显示「识别中」；「分析／沙盘／回复」显示分阶段进度条——阶段标签＋进度条＋已耗时秒数，阶段间进度条自动缓动爬升，判断结果先出、候选生成期间进度条持续走动。
