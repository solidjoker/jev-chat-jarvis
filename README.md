...
- 判断接口默认使用 TypeSafe 直连的 Jev（`https://api.typesafe.ai`，模型 `jev-latest`，与官方 quickstart 一致）；另有 OpenRouter / 博查 Jev / Vercel / 自定义可选。选博查后自动填好服务地址 `https://jev.bocha.cn` 与模型 `bocha-jev-v1`（协议与 TypeSafe 一致），页面上会显示官方地址并支持一键复制，当前限时免费。
...
- 判断接口有两个测试按钮：「测试意图判断」跑 7 项判断题；「测试沙盘推理」用内置示例回复验证候选排序，不依赖回复接口。回复、视觉各有一个连通测试按钮。
- 回复、视觉接口预设：智谱 GLM（`glm-5.3-flash`，默认）/ DeepSeek 官方（`deepseek-v4-flash`）/ 通义兼容 / 自定义，GLM 地址 `https://open.bigmodel.cn/api/paas/v4`。OpenRouter 等其他服务用「自定义」粘贴 `/v1` 地址即可。
- 三路都支持自定义地址：回复 / 视觉填任何 OpenAI 兼容地址（到 `/v1` 为止，或直接粘完整 `/chat/completions` 地址），局域网、本机模型服务（如 Ollama `http://192.168.x.x:11434/v1`）可以直接用；判断接口「自定义」档填完整 URL，原样 POST。
...
| 微信 | 已隐藏消息文字，部分设备还开启了防截屏 | 实验：气泡节点读取为主，读不到时用通知缓冲（对方新消息）替代 OCR；微信内永不截屏 |
...
- **微信为实验支持（1.5 起）**：正文对无障碍隐藏、屏幕又开了防截屏，所以微信内**绝不截屏**——优先读聊天气泡节点，读不到时通过系统通知读对方新消息（设置里授予「通知使用权」）。通知通道只覆盖对方新消息、看不到历史和自己发的；正在微信里打开的那个会话不会有通知。填入失败退回复制到剪贴板。
